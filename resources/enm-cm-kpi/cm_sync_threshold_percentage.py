...
def check_cm_sync_status(session):
    ...
    cm_supervision_cmd = ('cmedit get * CmNodeHeartbeatSupervision.active==true')
    out = run_enm_command(session, cm_supervision_cmd)
    if not any(cm_succ_str in str(line) for line in out):
        raise RuntimeError("Command '{}' was not successful. "
                           "Output={}".format(cm_supervision_cmd, out))
    supervised = out[-1].value().strip(cm_succ_str)
    LOGGER.info("Synchronized nodes count: '%s' \nSupervised nodes count: '%s'",
                synchronized, supervised)
    if int(supervised) == 0:
        LOGGER.info("Total CM Supervised node count is 0.")
        exit(1)
    synchronized_percent = percentage(synchronized, supervised)
    LOGGER.info("Percent of Synchronized nodes: '%s'", synchronized_percent)
    return synchronized_percent
def main():
    """
